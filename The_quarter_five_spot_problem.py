...
def injCorner(x, on_boundary): return x[0] == 0.0 and x[1] == 0.0

def prodCorner(x, on_boundary): return x[0] == 1.0  and x[1] == 1.0
...
bc_pC = DirichletBC(W.sub(1),Constant(-0.25),prodCorner,method = "pointwise")
...
outfileV = File('FiveSpotProblem_Velocity.pvd')
outfileP = File('FiveSpotProblem_Pressure.pvd')
U = Function(W)
solve(a == L, U, bcs)
